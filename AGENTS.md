...
- Current phase state:
  ...
  - External browser-game QA:
    ...
    - Current active local builder handoff: Deckbuilder Upgrade Phase 2: Enemy Intent And Status Explainability through deckbuilder builder `019ef96e-7780-7763-b444-12cf7698a97a`. Scope is original intent icon+label+number chips, visible enemy intent resolution feedback, Focus/Fractured help by mouse and keyboard, card consequence previews that include status effects, optional next-intent preview, README updates, and preservation of audio-off defaults. Do not hand this to the canonical tester until the builder reports completion.
