...
- Current phase state:
  ...
  - External browser-game QA:
    ...
    - Current active local QA handoff: Arcade Kart Racer settings-depth retest 2 through the canonical tester, evidence target `evidence/kart-racer/settings-depth-retest-2/`, after browser maintenance closed stale tabs and reported the shared harness healthy enough to retry.
    - Current local build awaiting QA: Side-Scrolling Platformer Settings Polish 2 is builder-complete and queued behind the active Kart retest for canonical tester QA. It adds deeper player settings for accessibility/readability, controls/feel, difficulty/forgiveness, and display/performance. Do not mark approved until the canonical tester writes passing evidence.
