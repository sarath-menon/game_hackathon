...
- Current phase state:
  ...
  - External browser-game QA:
    ...
    - Current active external QA handoff: Dungeons & Degenerate Gamblers shared-window canonical retry 1 through the canonical tester, evidence target `evidence/external/dungeons-degenerate-gamblers-shared-window-retry-1/`.
    - Reason for active Dungeons retry: the older Dungeons report predates the current shared-window rule and used an isolated browser profile; this pass should deepen card-state, legal-action, settings/options, and first-encounter QA methodology.
