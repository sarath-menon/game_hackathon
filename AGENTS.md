...
- Current phase state:
  ...
  - External browser-game QA:
    ...
    - Dungeons & Degenerate Gamblers Direct Official Build Retry 1 is `PASS_WITH_FINDINGS` at `evidence/external/dungeons-degenerate-gamblers-direct-build-retry-1/`: the direct official itch-hosted build reached playable run state, deck selection, tavern/node entry, first table HIT/STAND actions, settings/options, pause/menu controls, and restart boundary attempts. Remaining nonblocking findings: deeper illegal/unavailable action feedback and restart/give-up confirmation copy need a future focused pass.
    - Current active external QA handoff: Dungeons & Degenerate Gamblers Direct Official Build Follow-up 2 through canonical tester `019ef96e-99ee-7f62-b4d2-7d2c3cd29217`, evidence target `evidence/external/dungeons-degenerate-gamblers-legality-restart-pass-1/`, using URL `https://html-classic.itch.zone/html/15020933/index.html`. Scope is turn-phase legality, invalid/unavailable action feedback, HIT/STAND state transitions, restart/give-up/save-and-quit confirmation/cancel copy, settings/menu smoke, and provider/environment classification. Do not start another browser QA run until this tester pass closes or blocks.
