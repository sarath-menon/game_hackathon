...
- Current phase state:
  ...
  - External browser-game QA:
    ...
    - Cookie Clicker deeper settings/persistence/narrow-viewport pass 1 is `PASS_WITH_FINDINGS` at `evidence/external/cookie-clicker-settings-persistence-mobile-pass-1/`: direct provider load, language selection, banner dismissal, click/purchase loop, Options access/scrolling, export-save access, expected-flow, and MP4 evidence were completed; persistence/reload and narrow/mobile remained documented coverage gaps from the interrupted run.
    - Cookie Clicker Follow-up Pass 2 is `PASS_WITH_FINDINGS` at `evidence/external/cookie-clicker-followup-pass-2/`: direct provider stayed playable, cookie clicks and a safe Grandma purchase created distinctive state, manual Save plus reload verified persistence, Options coverage expanded to export/import safe modals, save/load file affordances, language, volume/audio, visual/performance toggles, warning toggles, screen reader mode, mod data, and separated Wipe save controls. True mobile emulation remains a low environment-limited finding because the browser-harness CDP helper blocked device metrics, but the 782 x 859 narrow desktop viewport remained usable.
    ...
    - Townscaper Removal, Undo, Gear/Settings, And Placement-Clarity Follow-Up 1 is `PASS_WITH_FINDINGS` at `evidence/external/townscaper-removal-undo-settings-followup-1/`: direct WebGL provider reached playable state, outlined-cell placement/removal, zoom, and camera drag were verified; gear/help, undo/redo, target affordance, color-only controls, and startup delay remain nonblocking findings.
    - Cookie Clicker Accessibility, Narrow-Viewport, And Settings Persistence Follow-Up 3 is currently active with the canonical tester at `evidence/external/cookie-clicker-accessibility-narrow-followup-3/`; classify provider/environment/viewport blockers separately from game QA failures.
    ...
    - Dungeons & Degenerate Gamblers Follow-up Pass 2 is `BLOCKED_ENVIRONMENT` at `evidence/external/dungeons-degenerate-gamblers-settings-illegal-pass-1/`: the official itch provider and game menu loaded, but the shared-window browser-harness viewport could not reach the clipped `New Run` control after same-window recovery attempts. This is not a game `FAIL` and not `BLOCKED_PROVIDER`.
    - Dungeons & Degenerate Gamblers Direct Official Build Retry 1 is `PASS_WITH_FINDINGS` at `evidence/external/dungeons-degenerate-gamblers-direct-build-retry-1/`: the direct official itch-hosted build reached playable run state, deck selection, tavern/node entry, first table HIT/STAND actions, settings/options, pause/menu controls, and restart boundary attempts.
...
- Game and phase
- Verdict: local-game phase reports use `PENDING`, `PASS`, or `FAIL`; external-game reports use `PASS_WITH_FINDINGS`, `FAIL`, `BLOCKED_PROVIDER`, `BLOCKED_ENVIRONMENT`, or `UNTESTABLE_IN_CURRENT_ENVIRONMENT`
