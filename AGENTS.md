...
- Current phase state:
  ...
  - External browser-game QA:
    ...
    - Current active local QA handoff: Side-Scrolling Platformer Settings Polish 2 through the canonical tester, evidence target `evidence/platformer/settings-polish-2/`. It adds deeper player settings for accessibility/readability, controls/feel, difficulty/forgiveness, and display/performance. Do not mark approved until the canonical tester writes passing evidence.
